...
verify_download_water <- function(tile_url, local_path) {
...
download_tile_water <- function(tile_url, local_path) {
...
#' }
download_tiles_water <- function(tiles, output_folder,
                           images=c('occurrence', 'change', 'seasonality', 
                                    'recurrence','transitions','extent')
                           ) {
    stopifnot(all(images %in% c('occurrence', 'change', 'seasonality', 
                                'recurrence','transitions','extent')))
    ...
    for (n in 1:length(tiles)) {
        ...
        if (min_x < 0) {
            min_x <- paste0(sprintf('%02i', abs(min_x)), 'W')
        } else {
            min_x <- paste0(sprintf('%02i', min_x), 'E')
        }
        if (max_y < 0) {
            max_y <- paste0(sprintf('%01i', abs(max_y)), 'S')
        } else {
            max_y <- paste0(sprintf('%01i', max_y), 'N')
        }
        file_suffix <- paste0('_', min_x, '_', max_y, '.tif')
        filenames <- paste0(images,  file_suffix)
        tile_urls <- paste0(paste0('http://storage.googleapis.com/global-surface-water/downloads/', images, '/'), filenames)
